...
@pytest.mark.skipif(hasattr(os, "lchmod"), reason="os.lchmod exists on this system")
async def test_lchmod_unsupported(tmp_path):
    """lchmod() raises NotImplementedError where the OS lacks it."""
    path = Path(tmp_path) / "file"
    await path.touch()
    with pytest.raises(NotImplementedError):
        await path.lchmod(0o600)
